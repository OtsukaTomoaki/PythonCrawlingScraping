...
def main():
    ...
    #ページを集めうrPageAggregatorオブジェクトを作成
    ...
    with open(sys.argv[1], 'rb') as f: #ファイルをバイナリ形式で開く
        ...
        #時間がかかるファイルは、キーワード引数pagenosで処理するページ番号（0始まり）のリストを指定する
...
def find_textboxes_recursively(component: LTComponent) -> List[LTTextBox]:
    ...
    '''
    #LTTextBoxを継承するオブジェクトの場合は１要素のリストを返す
    if isinstance(component, LTTextBox):
        return [component]
    #LTContainerを継承するオブジェクトは子要素を含むので再帰的に探す
